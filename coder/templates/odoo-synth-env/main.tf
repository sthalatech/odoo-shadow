...
data "coder_workspace_preset" "default_profile" {
  ...
  description = "Odoo + local Postgres hydrated from the newest masked dump, with the prs-backend (uat) addons repo live-mounted for development."
  parameters = {
    odoo_image       = "676206949426.dkr.ecr.us-east-1.amazonaws.com/odoo-synth/odoo:prof_749c8a90-fc88dea8aeef"
    dump_s3_uri      = "s3://odoo-synth-dumps-676206949426/masked-dumps/b6ad9d1f24e9/masked.dump"
...
resource "coder_agent" "main" {
  ...
  startup_script          = <<-EOT
    ...
    if [ -n "$ODOO_IMAGE" ]; then
      ...
      # Default SSO config placeholders: the isha_sso_auth_generic addon reads
