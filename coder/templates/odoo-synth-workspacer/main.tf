...
locals {
  ...
  # Same expose opt-in, but for a shared-infra dependency's own sidecar
  # (e.g. Celery Flower riding on the "redis" dependency's broker) rather
  # than a component -- dependencies are otherwise invisible infra, never
  # meant for a dashboard tile, so this is opt-in per dependency too.
  exposed_dependencies = {
    for d in try(local.components_decoded.dependencies, []) : d.name => d
    if try(d.expose.port, null) != null
  }
}
...
# Same mechanism as coder_app.component above, for a shared-infra
# dependency's own sidecar (e.g. Celery Flower, started as dep-flower by
# the boot script's dependency case above) that has expose.port set.
resource "coder_app" "dependency" {
  for_each     = local.exposed_dependencies
  agent_id     = coder_agent.main.id
  slug         = each.key
  display_name = each.key
  url          = "http://localhost:${each.value.expose.port}${lookup(each.value.expose, "path", "")}"
