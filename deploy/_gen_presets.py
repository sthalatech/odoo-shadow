...
to hydrate from). Written to coder/templates/odooshadow-workspacer/presets.tf, which
...
OUT = REPO / "coder" / "templates" / "odooshadow-workspacer" / "presets.tf"
...
def _latest_mask_dump(profile_id: str) -> tuple[float, str] | None:
    """(timestamp, S3 URI) of the masked dump from the most recent succeeded
    mask run for a profile, or None if there isn't one yet.
    ...
    key. Returns the newest *valid* one."""
    ...
    for r in run_store.list_runs(limit=500):
        ...
        if not uri or not uri.endswith("masked.dump"):
            continue
        ts = float(full.get("finished_at") or full.get("created_at") or 0)
...
def main() -> int:
    ...
    for p in profile_store.list_profiles(limit=500):
        ...
        dump_info = _latest_mask_dump(p["id"])
        if not dump_info:
            continue
        mask_ts, dump_uri = dump_info
        ...
        mask_when = (datetime.fromtimestamp(mask_ts, tz=timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
                     if mask_ts else "unknown time")
        # One preset per profile, refreshed on every mask (not one-per-run --
        # `coder templates push` already creates a new template version each
        # time, and only the active version's presets are offered for new
        # workspaces, so accumulating historical presets here would just be
        # clutter). The timestamp makes staleness visible in the dashboard.
        preset_name = f"{label} (masked {mask_when})"
